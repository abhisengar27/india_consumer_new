...
declare var Ladda
@Component({
    selector: 'personalContactInfoVehicleLoan',
    templateUrl: './personalContactInfoVehicleLoan.component.html'

})
export class PersonalContactInfoVehicleLoan implements AfterViewInit, OnInit {
    ...
    public date:string;
    public date_yr:number;
    public ShowMonths:boolean=true;
    ...
    constructor(private oaoService: OAOService, private router: Router, private route: ActivatedRoute, private gaEventsService: GoogleAnalyticsEventsService) {
        ...
        if (this.model.ResidenceType === null || this.model.ResidenceType === '' || this.model.ResidenceType === undefined) {
           ...
           if (this.model.currentResidence === null || this.model.currentResidence === undefined) {
            this.model.currentResidence = 'Select year';
        }

    }

        change(){
        console.log("on change of Work_experience"+this.model.currentResidence);
        var Year =parseInt(this.model.currentResidence);
        console.log(this.model.currentResidence);
        if(Year > this.date_yr-4){
           this.ShowMonths= false;
        }
        else{
            this.ShowMonths= true;
            console.log("selected year"+this.date_yr)
        }
    }
    ...
    ngOnInit() {
        console.log("date"+this.date)
        this.date_yr= parseInt(this.date.substring(11, 15));
          console.log("date"+this.date_yr);
